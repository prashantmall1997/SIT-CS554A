...
const SeriesList = (props) => {
  ...
  const [seriesList, setSeriesList] = useState(undefined);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const baseUrl = `https://gateway.marvel.com:443/v1/public/series?limit=100&offset=${offSet}`;
        const url =
        ...
        setSeriesList(marvel.data.results);
        setLoading(false);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const stringToHash = ts + privatekey + publickey;
        const hash = md5(stringToHash);
        const baseUrl = `https://gateway.marvel.com:443/v1/public/series?limit=100&titleStartsWith=${searchTerm}`;
        const url =
          baseUrl + "&ts=" + ts + "&apikey=" + publickey + "&hash=" + hash;
  ...
  if (searchTerm) {
    list =
      ...
      searchData.map((series) => {
        return buildList(series);
      });
  } else if (seriesList) {
    // return buildList(seriesList[0]);
    list = seriesList.map((character) => {
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        <Link to="/series/page/0">Click here to go back to first page</Link>
      </div>
    );
  } else if (searchTerm !== "") {
    return (
