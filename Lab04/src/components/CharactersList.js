...
const CharactersList = (props) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const baseUrl = `https://gateway.marvel.com:443/v1/public/characters?limit=100&offset=${offSet}`;
        const url =
        ...
        setCharactersList(marvel.data.results);
        setLoading(false);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const stringToHash = ts + privatekey + publickey;
        const hash = md5(stringToHash);
        const baseUrl = `https://gateway.marvel.com:443/v1/public/characters?limit=100&nameStartsWith=${searchTerm}`;
        const url =
          baseUrl + "&ts=" + ts + "&apikey=" + publickey + "&hash=" + hash;
        console.log("URL is --> " + url);
        const { data: marvel } = await axios.get(url);
  ...
  if (searchTerm) {
    ...
    list = charactersList.map((character) => {
      return buildList(character);
    });
  }
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        <Link to="/characters/page/0">Click here to go back to first page</Link>
      </div>
    );
  } else if (searchTerm !== "") {
    return (
